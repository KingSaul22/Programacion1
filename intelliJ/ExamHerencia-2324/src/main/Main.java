...
public class Main {
    public static void main(String[] args) {
        ...
        for (Pokemon pokemon : pokemons) {
            System.out.println(pokemon.getNombre() + " - " + pokemon.getClass().getSimpleName() + " - " + pokemon.getSalud() + " puntos de salud");
        }
            ...
            for (int i = 0; i < pokemons.length; i++) {
                ...
                if (pokemon.estaVivo()) {
                    ...
                    try {
                        ...
                        System.out.println(pokemon.getNombre() + " ha atacado a " + ((Pokemon) atacable).getNombre() + " - " + ((Pokemon) atacable).getSalud() + " puntos de salud");
    ...
    private static Atacable seleccionaPokemonAtacable(Pokemon[] pokemons, int i) {
        ...
        for (int j = 0; j < pokemons.length; j++) {
            // Comprobar que el pokemon está vivo, que no es él mismo y que puede ser atacado
            if (pokemons[j].estaVivo() && j != i && pokemons[j] instanceof Atacable) {
                System.out.println((numPokemonVivos + 1) + ": " + pokemons[j].getNombre() + " - " + pokemons[j].getClass().getSimpleName() + " - " + pokemons[j].getSalud() + " puntos de salud");
                numPokemonVivos++;
    ...
    private static WeatherCondition generateRandomWeatherCondition() {
        ...
        return switch (random) {
            //Devolver un valor de WeatherCondition
